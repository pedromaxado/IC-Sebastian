...
long long int AlgHeap( int n, long long int *vecX, long long int *vecY, bool **mtx, int* mina, int* minb, heap h ) {
    ...
    visited_pairs.insert(make_pair(p.aind, p.bind));
    // stack[0].aind=0;
    // stack[0].bind=0;
    // stsize=1;
    // hash[0][0]=0;
        ...
        if ( mtx[p.aind][p.bind] ) {
            ...
            *minb=p.bind;
            ...
            if ( p.aind+1 < n && visited_pairs.find(make_pair(p.aind+1, p.bind)) == visited_pairs.end() ) {
                ...
                h_insert(&h, q);
                visited_pairs.insert(make_pair(q.aind, q.bind));
